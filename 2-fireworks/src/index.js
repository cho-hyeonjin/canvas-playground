...
class Canvas extends CanvasOption {
  ...
  createTail() {
    ...
    const color = "255, 255, 255"; // Particle과 색상값을 공유해야 하므로, Particle에서 draw에 사용된 color값 형식과 통일시켜준다.
    ...
    const frame = () => {
      ...
      this.tails.forEach((tail, index) => {
        ...
        if (tail.vy > -0.7) {
          ...
          this.createParticles(tail.x, tail.y, tail.color); // tail 마찰력이 1에 수렴하는 부분에서 particle이 생성되게끔 하기 위해 인자값으로 해당 위치 좌표값 전달, (시각적으로 Tail 색상과 Particle 색상이 동일해야 쏘아올린 폭죽이 터지는 것으로 인식될 것이므로) 컬러값도 전달
        }
      });
